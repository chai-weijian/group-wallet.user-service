...
package com.chaiweijian.groupwallet.userservice.group.invitation.accept;
...
@Component
public class AcceptGroupInvitationRequestProcessor {

    private final KafkaProtobufSerde<GroupInvitation> groupInvitationSerde;

    public AcceptGroupInvitationRequestProcessor(KafkaProtobufSerde<GroupInvitation> groupInvitationSerde) {
        this.groupInvitationSerde = groupInvitationSerde;
    }

    @Bean
    public BiFunction<KStream<String, AcceptGroupInvitationRequest>, GlobalKTable<String, GroupInvitation>, KStream<String, Status>> acceptGroupInvitation() {
        return (acceptGroupInvitationRequest, groupInvitationStore) -> {

            var joined = acceptGroupInvitationRequest.leftJoin(groupInvitationStore,
            ...
            var accepted = groupInvitationStateValidation.getPassedStream()
            ...
            accepted.to("groupwallet.userservice.GroupInvitationAccepted-events", Produced.with(Serdes.String(), groupInvitationSerde));

            var successStatus = accepted.mapValues(value -> OkStatusUtil.packStatus(value, "Group Invitation accepted"));
    ...
    private static class RequestAndGroupInvitation{
        private final AcceptGroupInvitationRequest request;
